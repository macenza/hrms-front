...
import { Edit2, Trash2, Power, Loader2, FolderClosed, Calendar, User } from 'lucide-react';
...
interface AssetCategoryTableProps {
    ...
    isLoading?: boolean;
    onEdit: (record: AssetCategory) => void;
    onToggleStatus: (record: AssetCategory) => void;
    onDelete: (record: AssetCategory) => void;
}
...
export default function AssetCategoryTable({
    ...
    const isInitialLoad = isLoading && categories.length === 0;
                            ...
                            <td colSpan={5} className="px-6 py-16 text-center text-gray-500 dark:text-gray-400 transition-colors">
                                <div className="flex flex-col items-center justify-center">
                                    <div className="w-12 h-12 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center mb-3 transition-colors">
                                        <FolderClosed size={24} className="text-gray-400 dark:text-gray-500" />
                                    </div>
                                    <p className="font-semibold text-gray-900 dark:text-gray-100 text-base transition-colors">No categories found</p>
                                    <p className="text-sm mt-1">There are no asset categories defined yet.</p>
                                </div>
                            </td>
                        </tr>
                    ) : (
                        categories.map((rec) => (
                            <tr key={rec._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
                                <td className="px-6 py-4 flex items-center gap-4 max-w-sm truncate">
                                    <div className={cn(
                                        "p-2.5 rounded-lg shadow-sm dark:shadow-none transition-colors",
                                        rec.isActive 
                                    ...
                                    <div className="truncate">
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 transition-colors truncate max-w-xs" title={rec.description}>
                                ...
                                <td className="px-6 py-4 text-gray-600 dark:text-gray-300 font-medium transition-colors">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span>{rec.createdBy?.name || 'System'}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-gray-600 dark:text-gray-300 font-medium transition-colors">
